...
Suite: **268 passing, 0 failing, 160 skipped, 428 total.** The 139 pre-existing tests are
unmodified and all pass. 129 of the passing tests are new adversary regression guards.
...
attributes stripped gives 160 failures and 129 passes — the same 160. Reproduce with:
...
| kind | count |
|---|---|
| New defect in an existing feature — not on the §7 list | **79** |
...
|---|---|---|---|
| 61 | Modifiers | `     int f;` — `NoAccessibility` leaves a stray space | `    int f;` |
| 62 | Modifiers | `public readonly static int f` | `public static readonly int f` |
| 63 | Structure | a blank line before the **first** member of every type | none |
| 64 | Output context | `using Aaa; using System.Text; using Zzz;` | `System` namespaces first |
| 65 | Type model | `TypeDefinition.Get(typeof(int)).ToString()` is `.int` | `int` |
| 66 | Structure | `CloseScope()` past zero throws `ArgumentOutOfRangeException: count '-4'` | a diagnosis of the unbalanced scope |
...
| `Adversary/TriviaAdversaryTests.cs` | 5 | 6 |
