...
| B11 | **`for(`, `foreach(` and `while(` gain the space that `if (` and `switch (` already had.** | landed, post-preview1003 | None — whitespace only, and it makes one emitted file internally consistent rather than having two spellings of the same thing in it. But it is **cosmetic and repo-wide**, like B6: every snapshot containing a loop diffs at once, for no semantic reason. Two tests in this repo were updated with it. Worth landing in its own commit so the snapshot review stays legible. |
| B12 | **A member declared `ComponentModifier.NoAccessibility` loses a leading space, and a constructor's `: base(...)` loses one.** | landed, post-preview1003 | None — both were off-by-one indents. `partial void OnCreated();` sat one column right of every other member in its type, and `: base(id)` one column right of the four-space continuation it was aiming for. No snapshot in this repo encoded either, which is why neither was noticed. |
