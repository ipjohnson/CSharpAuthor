...
public static class SyntaxHelpers
{
    ...
    /// <summary>
    /// An ordinary <c>//</c> comment written above the declaration, before its documentation
    /// comment and before anything the file derives.
    /// </summary>
    /// <remarks>
    /// On a <see cref="CSharpFileDefinition"/> this is the file prologue: it lands above the
    /// generated <c>using</c> directives, which is the only place a generated-file header is any
    /// use. <see cref="BaseOutputComponent.Comment"/> cannot do this - it writes <c>///</c>, which
    /// is XML and documents whatever follows it.
    /// </remarks>
    public static void AddHeaderComment(this BaseOutputComponent baseOutputComponent, string comment)
    {
        baseOutputComponent.AddLeadingTrait(new LineCommentComponent(comment));
    }

    /// <summary>
    /// The <c>&lt;auto-generated/&gt;</c> header, which is what stops analyzers and style tools
    /// reporting on a generated file.
    /// </summary>
    /// <remarks>
    /// The marker has to be in the first comment of the file for the compiler to honour it, which
    /// is why this is a leading trait rather than something written into the body.
    /// </remarks>
    public static void AddAutoGeneratedHeader(this BaseOutputComponent baseOutputComponent)
    {
        baseOutputComponent.AddHeaderComment("<auto-generated/>");
    }

    /// <summary>
    /// A <c>//</c> comment as a statement, at the block's indent.
    /// </summary>
    public static LineCommentComponent AddLineComment(
        this BaseBlockDefinition blockDefinition, string comment)
    {
        return blockDefinition.Add(new LineCommentComponent(comment));
    }
}