...
public class TriviaAdversaryTests
{
    ...
    /// below them.
    /// </remarks>
    [Fact(Skip = "ADVERSARY GAP: GenerateUsingStatements inserts the usings at index 0 after the fact, so a leading trait carrying '// <auto-generated/>' is pushed below them and no longer marks the file")]
    ...
    /// nothing.
    /// </summary>
    [Fact(Skip = "ADVERSARY GAP: CSharpFileDefinition does not override WriteComment, so a file-level comment set on it is silently dropped - the same defect EnumDefinition and EnumValueDefinition were fixed for")]
    ...
    [Fact(Skip = "ADVERSARY GAP: NamespaceDefinition does not override WriteComment either, so a comment on a namespace is dropped")]
